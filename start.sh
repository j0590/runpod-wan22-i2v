...
log "🔎 Verifying torch + sageattention..."
python -c "import torch; print('torch', torch.__version__, 'cuda', torch.version.cuda, 'available', torch.cuda.is_available())"
python -c "import sageattention; print('sageattention', getattr(sageattention,'__version__','unknown'))" || true
log "🚀 Validating ComfyUI installation..."
if [ ! -d "$COMFY_DIR" ] || [ -z "$(ls -A "$COMFY_DIR" 2>/dev/null)" ]; then
...
clone_repo(){ local name="$1"; local url="$2"; local path="$CUSTOM_NODES/$name"; if [ -d "$path" ]; then log "   Exists: $name"; return 0; fi; log "   Cloning: $name"; git clone --depth=1 --recursive "$url" "$path" >/dev/null 2>&1 || log "❌ Setup Failed: $name"; }
...
while IFS= read -r -d '' req_file; do
  tmp="$(mktemp)"
  grep -vE "^(torch|torchvision|torchaudio|opencv-python|opencv-python-headless|sageattention)([<>=! ].*)?$" "$req_file" > "$tmp" || true
  if [ -s "$tmp" ]; then python -m pip install -r "$tmp" --upgrade-strategy only-if-needed >/dev/null 2>&1 || true; fi
  rm -f "$tmp"
...
if [ -d "$CUSTOM_NODES/ComfyLiterals/web" ]; then
  ...
  if [ ! -e "$COMFY_DIR/web/extensions/ComfyLiterals" ]; then ln -s "$CUSTOM_NODES/ComfyLiterals/web" "$COMFY_DIR/web/extensions/ComfyLiterals" || true; fi
fi
