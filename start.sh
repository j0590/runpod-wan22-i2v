...
# 1. HEAREMEN MEMORY OPTIMIZATION (Essential for 14B Models)
TCMALLOC="$(ldconfig -p | grep -Po "libtcmalloc.so.\d" | head -n 1)"
export LD_PRELOAD="${TCMALLOC}"
export PATH="/opt/venv/bin:$PATH"

# 2. HEAREMEN SAGEATTENTION BUILD (Background Process)
...
if [ ! -d "$COMFY_DIR" ] || [ -z "$(ls -A "$COMFY_DIR")" ]; then
    echo "📦 Cloning ComfyUI to volume..."
    cp -a /ComfyUI/. "$COMFY_DIR/"
fi

# 4. INSTALL YOUR CUSTOM NODES
mkdir -p "$CUSTOM_NODES"
cd "$CUSTOM_NODES"

echo "🧩 Installing Custom Nodes..."
# Helper to clone and install reqs
install_node() {
    local url="$1"
    ...
    if [ ! -d "$dir" ]; then
        ...
        if [ -f "$dir/requirements.txt" ]; then
            # Filter out torch to prevent version conflicts
...
# Adding Hearemen's Essentials that you likely need for workflows
...
wait # Wait for node requirements to install

# 5. DOWNLOAD YOUR MODELS
# Smart download function with Resume + Auth
download() {
    local url="$1"
    local out="$2"
    local auth="${3:-}"
