...
mkdir -p "$COMFY_DIR"
if [ ! -d "$COMFY_DIR" ] || [ -z "$(ls -A "$COMFY_DIR" 2>/dev/null || true)" ]; then log "📥 Initializing ComfyUI from Docker image..." && cp -a /ComfyUI/. "$COMFY_DIR"/; else log "✅ ComfyUI found in /workspace"; fi
mkdir -p "$CUSTOM_NODES"
clone_repo(){ local name="$1"; local url="$2"; local path="$CUSTOM_NODES/$name"; if [ -d "$path/.git" ]; then log "   Exists: $name"; return; fi; rm -rf "$path" || true; log "   Cloning: $name"; git clone --depth=1 --recursive "$url" "$path" >/dev/null 2>&1 || (rm -rf "$path" || true; log "❌ Setup Failed: $name"); }
...
find "$CUSTOM_NODES" -maxdepth 2 -name requirements.txt | while read -r req_file; do grep -vE "torch|torchvision|torchaudio|opencv-python|opencv-python-headless|sageattention" "$req_file" > "${req_file}.clean" || true; if [ -s "${req_file}.clean" ]; then python -m pip install --no-deps --no-cache-dir -r "${req_file}.clean" >/dev/null 2>&1 || true; fi; rm -f "${req_file}.clean"; done
mkdir -p "$LORA_DIR"
