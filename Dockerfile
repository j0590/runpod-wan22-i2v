FROM --platform=linux/amd64 nvidia/cuda:12.8.1-cudnn-devel-ubuntu24.04
...
RUN python -c "import platform; print('arch', platform.machine())" \
...
RUN git clone --depth=1 https://github.com/thu-ml/SageAttention.git /tmp/SageAttention \
    ...
    && python -c "import sageattention; print('sageattention', getattr(sageattention,'__version__','unknown'))" \
