...
log "🔎 Torch/SageAttention sanity check (non-fatal)..."
python -c "import torch; print('torch', torch.__version__, 'cuda', torch.version.cuda)" || true
python -c "import sageattention; print('sageattention import ok')" || true
log "🚀 Validating ComfyUI installation..."
mkdir -p "$COMFY_DIR"
if [ -z "$(ls -A "$COMFY_DIR" 2>/dev/null || true)" ]; then
...
find "$CUSTOM_NODES" -maxdepth 2 -name requirements.txt | while read -r req_file; do grep -vE "torch|torchvision|torchaudio|opencv-python|opencv-python-headless|sageattention" "$req_file" > "${req_file}.clean"; if [ -s "${req_file}.clean" ]; then pip install -r "${req_file}.clean" --no-deps >/dev/null 2>&1 || true; fi; rm -f "${req_file}.clean"; done
mkdir -p "$LORA_DIR"
